...
public final class HorizontalCalendarViewModel: ObservableObject {
	
	@Published var allDays: [CalendarDayModel] = []
	...
	public init(startDate: Date) {
		...
		var allDays = Date.dates(from: startDate, to: endDate).map {
			CalendarDayModel(date: $0, isSelected: false, hasContentAvailable: false, isAvailable: true)
		}
	...
	func dayStringForegroundColorFor(_ day: CalendarDayModel) -> Color {
	...
	func dateStringForegroundColorFor(_ day: CalendarDayModel) -> Color {
		if day.isAvailable == false {
	...
	func backgroundColorFor(_ day: CalendarDayModel) -> Color {
		if day.isSelected {
			return Color.blue
...
public extension HorizontalCalendarViewModel {
